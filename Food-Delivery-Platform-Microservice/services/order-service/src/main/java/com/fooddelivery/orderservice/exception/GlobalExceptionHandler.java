...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleCircuitOpen(CallNotPermittedException ex) {
        return buildResponse(HttpStatus.SERVICE_UNAVAILABLE,
                "Service temporarily unavailable (circuit breaker open): " + ex.getCausingCircuitBreakerName());
    }

    @ExceptionHandler(FeignException.NotFound.class)
    public ResponseEntity<Map<String, Object>> handleFeignNotFound(FeignException.NotFound ex) {
        return buildResponse(HttpStatus.NOT_FOUND, "Referenced resource not found in upstream service");
    }

    @ExceptionHandler(FeignException.class)
    public ResponseEntity<Map<String, Object>> handleFeignError(FeignException ex) {
        return buildResponse(HttpStatus.SERVICE_UNAVAILABLE,
                "Upstream service unavailable: " + ex.status());
    }
